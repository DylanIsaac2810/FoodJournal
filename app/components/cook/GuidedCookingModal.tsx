...
import { X, ChevronLeft, ChevronRight, Flame, CheckCircle2, Sparkles, Loader2, AlertCircle } from "lucide-react";
...
export function GuidedCookingModal({ recipe, onClose }: GuidedCookingModalProps) {
  ...
  const handleFinishCooking = async () => {
    ...
    try {
      ...
      console.error(error);
      // REEMPLAZO DEL ALERT POR NUESTRO TOAST BONITO
      showToast("Hubo un problema descontando los ingredientes de tu alacena.", "error");
    } finally {
  ...
  const toastMarkup = (
    <div 
      className={`fixed top-8 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl transition-all duration-300 ease-out transform ${
        toast.show ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0 pointer-events-none'
      } ${
        toast.type === 'error' ? 'bg-[#9E2A2B] text-white' : 'bg-[#335C67] text-[#FFF3B0]'
      }`}
    >
